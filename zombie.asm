...
lock_init(lock_t *lock)
{
  ...
  36:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
}
  3c:	5d                   	pop    %ebp
  3d:	c3                   	ret    
  3e:	66 90                	xchg   %ax,%ax

00000040 <lock_acquire>:
...
thread_create(void (*start_routine)(void *, void *), void* arg1, void* arg2)
{
  ...
  73:	53                   	push   %ebx
  74:	83 ec 20             	sub    $0x20,%esp
  void * freeStack = malloc(2*PGSIZE);
  77:	68 00 20 00 00       	push   $0x2000
  7c:	e8 ef 06 00 00       	call   770 <malloc>
  if(freeStack == 0) return -1;
  81:	83 c4 10             	add    $0x10,%esp
  84:	85 c0                	test   %eax,%eax
  86:	74 7f                	je     107 <thread_create+0x97>

  void *stack = freeStack;
  if((uint)freeStack % PGSIZE != 0)
  88:	89 c2                	mov    %eax,%edx
  8a:	89 c3                	mov    %eax,%ebx
  8c:	81 e2 ff 0f 00 00    	and    $0xfff,%edx
  92:	75 4c                	jne    e0 <thread_create+0x70>
    stack = freeStack + PGSIZE -((uint)freeStack % PGSIZE);

  int childPid = clone(start_routine, arg1, arg2, stack);
  94:	50                   	push   %eax
  95:	ff 75 10             	push   0x10(%ebp)
  98:	ff 75 0c             	push   0xc(%ebp)
  9b:	ff 75 08             	push   0x8(%ebp)
  9e:	e8 d0 03 00 00       	call   473 <clone>
  if(childPid != -1)
  a3:	83 c4 10             	add    $0x10,%esp
  a6:	83 f8 ff             	cmp    $0xffffffff,%eax
  a9:	74 45                	je     f0 <thread_create+0x80>
  {
    for(int i = 0; i < SIZE; i++)
  ab:	31 d2                	xor    %edx,%edx
  ad:	eb 09                	jmp    b8 <thread_create+0x48>
  af:	90                   	nop
  b0:	83 c2 01             	add    $0x1,%edx
  b3:	83 fa 40             	cmp    $0x40,%edx
  b6:	74 19                	je     d1 <thread_create+0x61>
    {
      if(userStackAddr[i] == NULL)
  b8:	8b 0c 95 40 0d 00 00 	mov    0xd40(,%edx,4),%ecx
  bf:	85 c9                	test   %ecx,%ecx
  c1:	75 ed                	jne    b0 <thread_create+0x40>
      {
        userStackAddr[i] = freeStack;
  c3:	89 1c 95 40 0d 00 00 	mov    %ebx,0xd40(,%edx,4)
        valueArray[i] = childPid;
  ca:	89 04 95 40 0c 00 00 	mov    %eax,0xc40(,%edx,4)
  {
    free(freeStack);
  }
  ...
  e7:	eb ab                	jmp    94 <thread_create+0x24>
  e9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    free(freeStack);
  f0:	83 ec 0c             	sub    $0xc,%esp
  f3:	89 45 f4             	mov    %eax,-0xc(%ebp)
  f6:	53                   	push   %ebx
  f7:	e8 e4 05 00 00       	call   6e0 <free>
 ...
 106:	c3                   	ret    
  if(freeStack == 0) return -1;
 107:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
 10c:	eb c3                	jmp    d1 <thread_create+0x61>
 ...
 115:	8d 45 f4             	lea    -0xc(%ebp),%eax
{
 118:	83 ec 1c             	sub    $0x1c,%esp
  int childPid = join(&stack);
 11b:	50                   	push   %eax
 11c:	e8 5a 03 00 00       	call   47b <join>
 ...
 124:	89 c6                	mov    %eax,%esi
  if(childPid != -1) {
 126:	83 f8 ff             	cmp    $0xffffffff,%eax
 129:	74 46                	je     171 <thread_join+0x61>
    for(int i=0; i < SIZE; i++) {
 12b:	31 db                	xor    %ebx,%ebx
 12d:	eb 09                	jmp    138 <thread_join+0x28>
 12f:	90                   	nop
 130:	83 c3 01             	add    $0x1,%ebx
 133:	83 fb 40             	cmp    $0x40,%ebx
 136:	74 39                	je     171 <thread_join+0x61>
      if(valueArray[i] == childPid) {
 138:	39 34 9d 40 0c 00 00 	cmp    %esi,0xc40(,%ebx,4)
 13f:	75 ef                	jne    130 <thread_join+0x20>
        free(userStackAddr[i]);
 141:	83 ec 0c             	sub    $0xc,%esp
 144:	ff 34 9d 40 0d 00 00 	push   0xd40(,%ebx,4)
 14b:	e8 90 05 00 00       	call   6e0 <free>
        valueArray[i] = -1;
        userStackAddr[i] = NULL;
 150:	83 c4 10             	add    $0x10,%esp
        valueArray[i] = -1;
 153:	c7 04 9d 40 0c 00 00 	movl   $0xffffffff,0xc40(,%ebx,4)
 15a:	ff ff ff ff 
        userStackAddr[i] = NULL;
 15e:	c7 04 9d 40 0d 00 00 	movl   $0x0,0xd40(,%ebx,4)
 165:	00 00 00 00 
    for(int i=0; i < SIZE; i++) {
 169:	83 c3 01             	add    $0x1,%ebx
 16c:	83 fb 40             	cmp    $0x40,%ebx
 16f:	75 c7                	jne    138 <thread_join+0x28>
      }
    }
  }
  return childPid;
...
free(void *ap)
{
 ...
 6e1:	a1 40 0e 00 00       	mov    0xe40,%eax
{
 ...
 721:	5b                   	pop    %ebx
  freep = p;
 722:	89 15 40 0e 00 00    	mov    %edx,0xe40
}
 728:	5e                   	pop    %esi
 729:	5f                   	pop    %edi
 72a:	5d                   	pop    %ebp
 ...
 759:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 75c:	89 15 40 0e 00 00    	mov    %edx,0xe40
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 779:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 77c:	8b 3d 40 0e 00 00    	mov    0xe40,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 7bf:	73 4f                	jae    810 <malloc+0xa0>
    ...
    if(p == freep)
 7c1:	8b 3d 40 0e 00 00    	mov    0xe40,%edi
 ...
 7ea:	e8 f1 fe ff ff       	call   6e0 <free>
  return freep;
 7ef:	8b 15 40 0e 00 00    	mov    0xe40,%edx
 ...
 81c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 81f:	89 15 40 0e 00 00    	mov    %edx,0xe40
}
 ...
 82f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
 830:	c7 05 40 0e 00 00 44 	movl   $0xe44,0xe40
 837:	0e 00 00 
    base.s.size = 0;
 83a:	bf 44 0e 00 00       	mov    $0xe44,%edi
    base.s.ptr = freep = prevp = &base;
 83f:	c7 05 44 0e 00 00 44 	movl   $0xe44,0xe44
 846:	0e 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 849:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 84b:	c7 05 48 0e 00 00 00 	movl   $0x0,0xe48
 852:	00 00 00 
    if(p->s.size >= nunits){
